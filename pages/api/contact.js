...
// Initialize Twilio client (fail gracefully if credentials missing)
...
<body>
    <div class="header">
        <h1>New Contact Submission ${twilioClient ? '<span class="whatsapp-badge">WhatsApp Sent</span>' : ''}</h1>
    </div>
    ...
    <div class="content">
        ...
        <div class="details-grid">
            ...
            <div>
                ...
                ${twilioClient ? `<a href="https://wa.me/${contactInfo.phone.replace('+', '')}" style="margin-left:10px;">📱 WhatsApp</a>` : ''}
            </div>
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Validation
    const errors = [];
    if (!contactInfo.name) errors.push('Name is required');
    if (!contactInfo.phone) errors.push('Phone is required');
    if (!contactInfo.message) errors.push('Message is required');
    ...
    if (twilioClient && process.env.YOUR_WHATSAPP_NUMBER) {
      try {
        await twilioClient.messages.create({
          body: `🚀 NEW LEAD: ${contactInfo.name}\n\n📞 ${contactInfo.phone}\n📌 ${contactInfo.category.toUpperCase()}\n\n📝 Message:\n${contactInfo.message}\n\n⏰ ${new Date().toLocaleString()}`,
    ...
    await transporter.sendMail({
      ...
      to: process.env.EMAIL_USER,
      replyTo: `"${contactInfo.name}" <no-reply@varunarealty.com>`,
      subject: `${whatsappSuccess ? '[WhatsApp Sent] ' : ''}New ${contactInfo.category} inquiry from ${contactInfo.name}`,
